...
- **T9 — Shared `BookingWizard` + Portal self-booking + Admin on-behalf booking:** 3-step `BookingWizard.vue` (delivery mode → doctor+service → date+slot); `Portal\BookingController` (customer self-serve, `createdByRole=Customer`); `Admin\BookingController` (staff on-behalf + quick-create customer via `AuthService::registerCustomer`); 4 new routes (`portal.booking.create/store`, `admin.booking.create/store`); Inertia error mapping (`withErrors(['booking' => …])`); success redirects to `portal.home`/`admin.dashboard` pending T10 repoint; Vitest wizard spec + feature tests.
