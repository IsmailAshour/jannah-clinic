...
## 7. حدود P1 (YAGNI صريح)

لا كيان `Payment`/`Receipt`، لا فوترة/إيصالات، لا سجل طبي، لا ولاء/عضويات، لا إشعارات/تذكيرات (SMS/بريد/واتساب)، لا تقييمات أطباء (rating_average عرض فقط بلا واجهة إدخال)، لا عروض/خصومات/كوبونات، لا استهلاك جلسات عضوية. الموعد ينتهي عند `requested/confirmed` بسعر محسوب. كل ذلك P2–P5 حسب الروادماب.
